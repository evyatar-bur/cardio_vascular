%% Part 2 - different controller constants
...
%% Computing mean pao with smaller Ki
...
mean_Pao_small_Ki = zeros(1,Heart_cycles);
...
for CycleIdx = 1 : Heart_cycles % main loop for each heart cycle
    ...
    % Computing mean pao
    [mean_Pao_small_Ki(CycleIdx),Vlv_1,Va_1,Vv_1,Plv_1,Pa_1,Pv_1,Pao_1,Qlv_1,Qp_1,Qv_1] = Pao_func(HR,Emax,Cv,Rp,Vlv_1,Va_1,Vv_1,Plv_1,Pa_1,Pv_1,Pao_1,Qlv_1,Qp_1,Qv_1);
    
    % Updating error,last error and error sum
    last_error = error;
    error = wanted_pao - mean_Pao_small_Ki(CycleIdx);
    error_sum = error_sum + error;
    
end

%% Computing mean pao with bigger Kd
...
% Allocating for speed
mean_Pao_big_Kd = zeros(1,Heart_cycles);
...
for CycleIdx = 1 : Heart_cycles % main loop for each heart cycle
    ...
    % Computing mean pao
    [mean_Pao_big_Kd(CycleIdx),Vlv_1,Va_1,Vv_1,Plv_1,Pa_1,Pv_1,Pao_1,Qlv_1,Qp_1,Qv_1] = Pao_func(HR,Emax,Cv,Rp,Vlv_1,Va_1,Vv_1,Plv_1,Pa_1,Pv_1,Pao_1,Qlv_1,Qp_1,Qv_1);
    
    % Updating error,last error and error sum
    last_error = error;
    error = wanted_pao - mean_Pao_big_Kd(CycleIdx);
    error_sum = error_sum + error;
    
end

%% Plotting the computed mean pao for bigger kp
...
plot(mean_pao)
hold on
plot(mean_Pao_big_Kp)

title('Mean pao as a function of heart cycles')
xlabel('Heart Cycle [au]')
ylabel('mean pao [mmHg]')

legend('Original PID controller','Bigger Kp')